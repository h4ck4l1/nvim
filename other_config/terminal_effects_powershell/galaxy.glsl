...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // 1. CHANGED: Lowered opacity significantly so the fog isn't so thick
  const float cloudOpacity = 0.15; 
  ...
  // 2. CHANGED: Crushed the intensity from 0.4 down to 0.03. 
  // This forces the "white fog" to become a very faint, dark gray.
  float nebulaIntensity = length(backgroundColor) * 0.03; 
  backgroundColor = vec3(nebulaIntensity);
